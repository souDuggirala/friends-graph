...
public class Graph {
	...
	public Graph(Scanner sc) {
		...
		while (sc.hasNextLine()) {
			...
			//*these two people are friends with each other and have mutual edges
			String p1 = st.nextToken();
			String p2 = st.nextToken();
			int i = map.get(p1);
			int j = map.get(p2);
			members[i].first = new Friend(j,members[i].first); //makes p2 (with index j) a friend of p1
			members[j].first = new Friend(i,members[j].first); //makes p1 (with index i) a friend of p2
